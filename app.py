...
class GameEngine:
    ...
    def update_ranking_logic(self):
        ...
        if db:
            try:
                docs = db.collection('users').order_by('score', direction=firestore.Query.DESCENDING).limit(50).stream()
                for doc in docs:
                    data = doc.to_dict()
                    nick = data.get('nickname', 'Guest')
                    
                    if nick == 'Guest': continue
    ...
    def claim_free_score(self, uid):
        with game_lock:
            ...
            update_payload = {
                'score': new_score, 
                'last_claim_round': self.round_id 
            }
...
def get_status():
    ...
    remaining_time = game.get_remaining_time() # game 변수 사용됨
